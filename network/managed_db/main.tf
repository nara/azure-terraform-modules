resource "azurerm_private_endpoint" "private_endpoint" {
  name                = module.naming_conv.private_endpoint_name
  ...
  private_service_connection {
    name                           = format("%s-conn", module.naming_conv.private_endpoint_name)
    private_connection_resource_id = var.db_managed_instance_id
...
resource "azurerm_private_dns_zone" "private_dns_zone" {
  name                = module.naming_conv.db_private_dns_zone_name
  resource_group_name = var.resource_group_name
}
...
resource "azurerm_private_dns_zone_virtual_network_link" "dns_zone_to_vnet_link" {
  name                  = format("%s-vnet-link", module.naming_conv.db_private_dns_zone_name)
  resource_group_name   = var.resource_group_name
